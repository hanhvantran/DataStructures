...
open class NSSetManipulator : SetManipulator {
  fileprivate var setToManipulate = NSMutableSet()
  ...
  open func setupWithObjectCount(_ objectCount: Int) -> TimeInterval {
    return Profiler.runClosureForTime() {
      ...
      for _ in 0 ..< objectCount {
        self.setToManipulate.add(self.generator.standardRandomString())
      }
    }
  }
  ...
  fileprivate func addObjects(_ count: Int) -> TimeInterval {
    ...
    let timed =  Profiler.runClosureForTime() {
      self.setToManipulate.addObjects(from: objectsArray)
    }
